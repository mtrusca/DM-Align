...
def align_words(source, target, model, tokenizer):
    data_examples = []
    my_device = torch.device('cpu')
    example = namedtuple('example', 'ID, text_a, text_b, label')
    data_examples.append(example(0, source, target, '0-0'))
    data_examples
    set_type = 'test'
    batchsize = 1
    max_seq_length = 128
    test_dataloader = create_Data_Loader(data_examples=data_examples, max_span_size=4, set_type='test', batchsize=1,
                                         max_seq_length=128, tokenizer=tokenizer)
    for step, batch in enumerate(test_dataloader):
        batch = tuple(t.to(my_device) for t in batch)
        input_ids_a_and_b, input_ids_b_and_a, input_mask, segment_ids_a_and_b, segment_ids_b_and_a, sent1_valid_ids, \
        sent2_valid_ids, sent1_wordpiece_length, sent2_wordpiece_length = batch
    with torch.no_grad(): decoded_results = model(input_ids_a_and_b=input_ids_a_and_b,
                                                  input_ids_b_and_a=input_ids_b_and_a,
...
def convert_examples_to_features(data_examples, set_type, max_seq_length, tokenizer, max_span_size, pad_token=0,
    ...
    for ex_index, example in enumerate(data_examples):
        ...
        if len(example.text_a.split()) > max_seq_length or len(example.text_b.split()) > max_seq_length:
            if set_type == 'train':
                continue
            else:
                example.text_a = ' '.join(example.text_a.split()[:max_seq_length])
                example.text_b = ' '.join(example.text_b.split()[:max_seq_length])
        ...
        inputs = tokenizer.encode_plus(example.text_b, example.text_a, add_special_tokens=True,
                                       max_length=max_seq_length)
        ...
        assert len(input_ids_a_and_b) == max_seq_length, "Error with input length {} vs {}".format(
            len(input_ids_a_and_b), max_seq_length)
        assert len(attention_mask) == max_seq_length, "Error with input length {} vs {}".format(len(attention_mask),
                                                                                                max_seq_length)
        assert len(token_type_ids_a_and_b) == max_seq_length, "Error with input length {} vs {}".format(
            len(token_type_ids_a_and_b), max_seq_length)
        assert len(sent1_valid_ids) == max_seq_length, "Error with input length {} vs {}".format(len(sent1_valid_ids),
                                                                                                 max_seq_length)
        assert len(sent2_valid_ids) == max_seq_length, "Error with input length {} vs {}".format(len(sent2_valid_ids),
                                                                                                 max_seq_length)
        label_s2t = np.zeros((max_seq_length, max_seq_length), dtype=int) - 1
        label_t2s = np.zeros((max_seq_length, max_seq_length), dtype=int) - 1
        left2right = {}
        ...
        for pair in alignment_pair:
            ...
            tag_idx = 0
            for d in range(min(len(target_id_list), max_span_size)):
                if d == len(target_id_list) - 1 or d == max_span_size - 1:
                    tag_idx += target_id_list[0] + 1
                else:
                    tag_idx += lenB - d
            if len(source_id_list) > max_span_size:
                label_s2t[source_id_list[args.max_span_size - 1]][max_span_size - 1] = tag_idx
                for my_idx in range(max_span_size, len(source_id_list)):
                    label_s2t[source_id_list[my_idx]][0] = tag_idx
            else:
                label_s2t[source_id_list[-1]][len(source_id_list) - 1] = tag_idx
            tag_idx = 0
            for d in range(min(len(source_id_list), max_span_size)):
                if d == len(source_id_list) - 1 or d == max_span_size - 1:
                    tag_idx += source_id_list[0] + 1
                else:
                    tag_idx += lenA - d
            if len(target_id_list) > max_span_size:
                label_t2s[target_id_list[args.max_span_size - 1]][max_span_size - 1] = tag_idx
                for my_idx in range(max_span_size, len(target_id_list)):
        ...
        # print(label_t2s)
        # sys.exit()
        features.append(Features(input_ids_a_and_b=input_ids_a_and_b, input_ids_b_and_a=input_ids_b_and_a,
                                 attention_mask=attention_mask, segment_ids_a_and_b=token_type_ids_a_and_b,
                                 segment_ids_b_and_a=token_type_ids_b_and_a, sent1_valid_ids=sent1_valid_ids,
                                 sent2_valid_ids=sent2_valid_ids, sent1_wordpiece_length=sent1_wordpiece_length,
                                 sent2_wordpiece_length=sent2_wordpiece_length, label_s2t=label_s2t,
                                 label_t2s=label_t2s))
    return features


def create_Data_Loader(data_examples, max_span_size, set_type='train', batchsize=16, max_seq_length=128,
                       tokenizer=None):
    data_features = convert_examples_to_features(data_examples, set_type, max_seq_length, tokenizer, max_span_size)
    ...
    if set_type == 'train':
        ...
        dataset = TensorDataset(all_input_ids_a_and_b, all_input_ids_b_and_a, all_input_mask, all_segment_ids_a_and_b,
                                all_segment_ids_b_and_a, all_sent1_valid_ids, all_sent2_valid_ids,
                                all_sent1_wordpiece_length, all_sent2_wordpiece_length, all_label_s2t, all_label_t2s)
        data_sampler = RandomSampler(dataset)
    else:
        dataset = TensorDataset(all_input_ids_a_and_b, all_input_ids_b_and_a, all_input_mask, all_segment_ids_a_and_b,
                                all_segment_ids_b_and_a, all_sent1_valid_ids, all_sent2_valid_ids,
                                all_sent1_wordpiece_length, all_sent2_wordpiece_length)
        data_sampler = SequentialSampler(dataset)
    dataloader = DataLoader(dataset, sampler=data_sampler, batch_size=batchsize)
    return dataloader


def load_model_aligment():
...
class Features(object):
    def __init__(self, input_ids_a_and_b, input_ids_b_and_a, attention_mask, segment_ids_a_and_b, segment_ids_b_and_a,
                 sent1_valid_ids, sent2_valid_ids, sent1_wordpiece_length, sent2_wordpiece_length, label_s2t,
                 label_t2s):
...
class NeuralWordAligner(nn.Module):
    def __init__(self, args):
        ...
        self.args = args
        # self.bert_model = AutoModel.from_pretrained('spanbert_hf_base/', output_hidden_states=True, output_attentions=True)
        self.bert_model = BertModel.from_pretrained(args.spanbert_path, # 'SpanBERT/spanbert-base-cased',
        ...
        self.mlp1 = nn.Sequential(nn.Linear(self.sim_feature_num, self.sim_feature_num), nn.PReLU(),
                                  nn.Linear(self.sim_feature_num, 1))
        self.FF_kernel_spanbert = nn.Sequential(
            *[nn.Linear((span_representation_dim * 3) * 4, (span_representation_dim * 3)), nn.PReLU(),
              nn.Linear((span_representation_dim * 3), (span_representation_dim * 3)), nn.PReLU(),
              nn.Linear((span_representation_dim * 3), self.sim_feature_num)])
            ...
            if args.use_transition_layer == 'True':
                for j in range(extended_length_B + 1):  # 0 is NULL state
                    for k in range(extended_length_B + 1):  # k is previous state, j is current state
                        tmp_dist_idx_list[j][k] = put_a_number_in_bin_and_binary(self.distortionDistance(k, j, len_B),
    ...
    def _input_likelihood(self,
        ...
        logits = logits.permute(1, 2, 0,
                                3).contiguous()  # shape: (sequence_length, max_span_width, batch_size, num_tags)
        hamming_cost = hamming_cost.permute(1, 2, 0,
                                            3).contiguous()  # shape: (sequence_length, max_span_width, batch_size, num_tags)

        if self.args.use_transition_layer == "False":
            alpha = Variable(torch.FloatTensor([[0.0 for _ in range(batch_size)]]).to(self.my_device),
                             requires_grad=True)  # 1 * batch_size
        else:
            alpha = Variable(torch.FloatTensor(np.zeros((1, batch_size, num_tags))).to(self.my_device),
                             requires_grad=True)  # 1 * batch_size * num_tags
        ...
        for j in range(sequence_length):
            ...
            broadcast_alpha = reversed_alpha.view(width, batch_size,
                                                  -1)  # shape: (max_span_width, batch_size, num_tags)
            ...
            if self.args.use_transition_layer == "False":
                ...
                alpha_along_arglabels = logsumexp(broadcast_alpha + (span_factors + span_costs) * tag_mask[:, 0, :],
                                                  dim=-1)
                ...
                for k in range(num_tags):
                    emit_score = ((span_factors[:, :, k] + span_costs[:, :, k]) * tag_mask[:, 0, k]).unsqueeze(
                        -1).repeat(1, 1, num_tags)
                    tran_score = T[:, k, :] * tag_mask[:, k, :]  # bsz * num_tags
                    if j < width:
                        tran_score = tran_score.unsqueeze(0).repeat(width, 1, 1)
                        tran_score[j:, :, :] = 0
                    # Logsumexp the scores over the num_tags axis.
                    alpha_along_arglabels.append(
                        logsumexp(broadcast_alpha + tran_score + emit_score, dim=-1, keepdim=True))
    ...
    def _joint_likelihood(self,
        ...
        for j in range(sequence_length):
            for d in range(min(max_span_width, j + 1)):
                ...
                if j >= 1 and self.args.use_transition_layer == 'True' and j - (
                        d + 1) >= 0 and valid_tag_mask.sum() > 0:
    ...
    def viterbi_tags(self,
        ...
        for logits_ex, tag_mask, sequence_length, transition_matrix in zip(logits, tag_masks, sequence_lengths, T):
            ...
            optimal_sequence = self.viterbi_decoder(emission_matrix, transition_matrix, sequence_length,
                                                    sum(tag_mask) - 1)
            predA = set()
            target_sent_length = int((self.max_span_size - 1) / 2 + (sum(tag_mask) - 1) / self.args.max_span_size)
            for i, stateID in enumerate(optimal_sequence):
                if stateID == 0:
                    continue
                stateID -= 1
                for d in range(1, self.max_span_size + 1):
    ...
    def _compose_span_representations(self, seq1, seq2):
        ...
        for d in range(self.max_span_size):
            seq1_spans_d = []
            for i in range(max_sent1_length - d):
                current_ngrams = seq1[:, i:i + d + 1, :]  # bsz * current_span_size * 768
                alpha = torch.matmul(self.attn_proj(current_ngrams),
                                     self.attn_embd(idx).view(-1, 1))  # bsz * span_size * 1
                alpha = F.softmax(alpha, dim=1)
                seq1_spans_d.append(torch.cat([current_ngrams[:, :1, :], current_ngrams[:, -1:, :],
                                               torch.matmul(alpha.permute(0, 2, 1), current_ngrams)], dim=-1))
            seq1_spans_d = [torch.zeros((bsz, 1, self.span_representation_dim * 3)).to(self.my_device)] * (
                        max_sent1_length - len(seq1_spans_d)) + seq1_spans_d
            ...
            for i in range(max_sent2_length - d):
                ...
                alpha = torch.matmul(self.attn_proj(current_ngrams),
                                     self.attn_embd(idx).view(-1, 1))  # bsz * span_size * 1
                alpha = F.softmax(alpha, dim=1)
                seq2_spans_d.append(torch.cat([current_ngrams[:, :1, :], current_ngrams[:, -1:, :],
                                               torch.matmul(alpha.permute(0, 2, 1), current_ngrams)], dim=-1))
            seq2_spans_d = [torch.zeros((bsz, 1, self.span_representation_dim * 3)).to(self.my_device)] * (
                        max_sent2_length - len(seq2_spans_d)) + seq2_spans_d
    ...
    def compute_sim_cube_FF_spanbert(self, seq1, seq2):
        ...
        sim_cube = torch.Tensor(seq1.size(0), self.sim_feature_num, seq1.size(1),
                                seq2.size(1))  # bsz * feature_num * len_seq1 * len_seq2
    ...
    def forward(
        ...
        for i in range(batch_size):
            for j in range(sent1_lengths[i] - 1):
                ...
                if diff > 1:
                    ...
                    # print(j,position)
                    all_hidden_states[i][position:position + 1, :] = torch.mean(
                        all_hidden_states[i][position:position + diff, :], dim=0, keepdim=True)
                    position = sent1_valid_ids[i][j] + sent2_wordpiece_length[i] + 1
                    # print(j,position)
                    # print(sent2_wordpiece_length)
                    # sys.exit()
                    all_hidden_states_b_and_a[i][position:position + 1, :] = torch.mean(
                        all_hidden_states_b_and_a[i][position:position + diff, :], dim=0, keepdim=True)
            # print(sent1_valid_ids[i])
            # print(sent1_valid_ids[i]+sent2_wordpiece_length[i]+1)
            sent1_valid_ids_part2 = (sent1_valid_ids[i] + sent2_wordpiece_length[i] + 1) * (
                        sent1_valid_ids[i] > 0).int()
            # print(sent1_valid_ids_part2)
            # sys.exit()
            seq1.append(
                torch.index_select(all_hidden_states[i], 0, sent1_valid_ids[i]).unsqueeze(0) + torch.index_select(
                    all_hidden_states_b_and_a[i], 0, sent1_valid_ids_part2).unsqueeze(0))
            for j in range(sent2_lengths[i] - 1):
                ...
                if diff > 1:
                    ...
                    all_hidden_states_b_and_a[i][position:position + 1, :] = torch.mean(
                        all_hidden_states_b_and_a[i][position:position + diff, :], dim=0, keepdim=True)
            sent2_valid_ids_part2 = (sent2_valid_ids[i] - sent1_wordpiece_length[i] - 1) * (
                        sent2_valid_ids[i] > 0).int()
            seq2.append(
                torch.index_select(all_hidden_states[i], 0, sent2_valid_ids[i]).unsqueeze(0) + torch.index_select(
                    all_hidden_states_b_and_a[i], 0, sent2_valid_ids_part2).unsqueeze(0))
        ...
        simCube_context_spanbert = self.compute_sim_cube_FF_spanbert(self.layer_norm(seq1_spans),
                                                                     self.layer_norm(seq2_spans))

        simCube_context_spanbert_A2B = F.pad(simCube_context_spanbert, (1, 0), 'constant', 0)
        output_both_A2B = self.mlp1(simCube_context_spanbert_A2B.permute(0, 2, 3, 1)).squeeze(
            -1)  # bsz * num_all_source_spans * (num_all_target_spans + 1), 0 is inserted at the head position

        simCube_context_spanbert_B2A = F.pad(simCube_context_spanbert.permute(0, 1, 3, 2), (1, 0), 'constant', 0)
        output_both_B2A = self.mlp1(simCube_context_spanbert_B2A.permute(0, 2, 3, 1)).squeeze(
            -1)  # bsz * num_all_target_spans * (num_all_source_spans + 1)
        ...
        for i in range(batch_size):
            for d in range(1, self.max_span_size + 1, 1):
        ...
        logits_A2B = output_both_A2B.view(batch_size, self.max_span_size, -1, num_tags_B)
        logits_A2B = logits_A2B.permute(0, 2, 1, 3)  # shape (batch_size, sequence_length, max_span_width, num_tags)
        logits_B2A = output_both_B2A.view(batch_size, self.max_span_size, -1, num_tags_A)
        logits_B2A = logits_B2A.permute(0, 2, 1, 3)

        # print(logits_A2B.size())
        # print(logits_B2A.size())

        batch_dist_idx_list_B = np.zeros((batch_size, num_tags_B, num_tags_B),
                                         dtype=int)  # the last embedding dim is for padding
        ...
        for d in range(batch_size):
            extended_length_B = self.max_span_size * sent2_lengths[d] - int(
                self.max_span_size * (self.max_span_size - 1) / 2)
            batch_dist_idx_list_B[d, :(extended_length_B + 1), :(extended_length_B + 1)] = self.transition_matrix_dict[
                extended_length_B]
            tag_masks_sentB[d, :(extended_length_B + 1), :(extended_length_B + 1)] = np.ones(
                (extended_length_B + 1, extended_length_B + 1))

            extended_length_A = self.max_span_size * sent1_lengths[d] - int(
                self.max_span_size * (self.max_span_size - 1) / 2)
            batch_dist_idx_list_A[d, :(extended_length_A + 1), :(extended_length_A + 1)] = self.transition_matrix_dict[
                extended_length_A]
            tag_masks_sentA[d, :(extended_length_A + 1), :(extended_length_A + 1)] = np.ones(
                (extended_length_A + 1, extended_length_A + 1))

        extracted_tensor_idx_B = Variable(torch.from_numpy(batch_dist_idx_list_B).type(torch.LongTensor)).to(
            self.my_device)
        transition_matrix_sentB = self.mlp2(self.distance_embedding(extracted_tensor_idx_B)).squeeze(-1)
        extracted_tensor_idx_A = Variable(torch.from_numpy(batch_dist_idx_list_A).type(torch.LongTensor)).to(
            self.my_device)
        ...
        tag_masks_sentB = Variable(torch.from_numpy(tag_masks_sentB).type(torch.FloatTensor)).to(
            self.my_device)  # bsz * num_tags_B * num_tags_B
        tag_masks_sentA = Variable(torch.from_numpy(tag_masks_sentA).type(torch.FloatTensor)).to(
            self.my_device)  # bsz * num_tags_A * num_tags_A
        ...
        if self.training:
            ...
            # sys.exit()
            numerator_A2B = self._joint_likelihood(logits_A2B, transition_matrix_sentB, gold_tags_sentB,
                                                   text_masks_sentA)
            # print(numerator_A2B)
            zeros = Variable(
                torch.zeros(batch_size, max_len1, self.max_span_size, num_tags_B).float().to(self.my_device))
            ...
            denominator_A2B = self._input_likelihood(logits_A2B, transition_matrix_sentB, text_masks_sentA,
                                                     hamming_cost, tag_masks_sentB)
            # print(denominator_A2B)
            loss_A2B = (denominator_A2B - numerator_A2B).sum()  # /batch_size

            gold_tags_sentA = labels_t2s[:, :max_len2, :]
            numerator_B2A = self._joint_likelihood(logits_B2A, transition_matrix_sentA, gold_tags_sentA,
                                                   text_masks_sentB)
            zeros = Variable(
                torch.zeros(batch_size, max_len2, self.max_span_size, num_tags_A).float().to(self.my_device))
            ...
            # hamming_cost += negative_ones_only
            # hamming_cost = (hamming_cost > 0).float()
            denominator_B2A = self._input_likelihood(logits_B2A, transition_matrix_sentA, text_masks_sentB,
                                                     hamming_cost, tag_masks_sentA)
            loss_B2A = (denominator_B2A - numerator_B2A).sum()  # /batch_size

            return loss_A2B + loss_B2A

        else:
            optimal_sequence_A2B = self.viterbi_tags(logits_A2B, transition_matrix_sentB, text_masks_sentA,
                                                     tag_masks_sentB, reverse=False)
            optimal_sequence_B2A = self.viterbi_tags(logits_B2A, transition_matrix_sentA, text_masks_sentB,
                                                     tag_masks_sentA, reverse=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--my_device", default='cpu', type=str)
    ...
    model, tokenizer = load_model_aligment()
    alignments = {}
